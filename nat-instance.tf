...
locals {
  ...
  cidr_block         = var.cidr_block != "" ? var.cidr_block : element(concat(data.aws_vpc.default.*.cidr_block,list("")),0)
}

resource "aws_security_group" "nat_instance" {
  count       = var.enabled == true && var.nat_instance_enabled == true ? 1 : 0
